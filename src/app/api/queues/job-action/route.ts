// Proxy for the Monitor's Queues page → POST
// {TRACKVID_API_URL}/system-admin/queues/job-action. The SystemAdmin token is read from
// the NextAuth session so it never reaches the browser.
...
export async function POST(req: Request) {
  ...
  const res = await fetch(`${apiBase.replace(/\/$/, '')}/system-admin/queues/job-action`, {
    method: 'POST',
    headers: {
