// Proxy for the Queues page's queue-level menu → POST
// {TRACKVID_API_URL}/system-admin/queues/queue-action (pause, resume, empty,
// reset, set-concurrency, add-job). The SystemAdmin token is read from the
// NextAuth session so it never reaches the browser.
...
export async function POST(req: Request) {
  ...
  const res = await fetch(`${apiBase.replace(/\/$/, '')}/system-admin/queues/queue-action`, {
    method: 'POST',
    headers: {
